...
class PatchTSTEncoderLayerWithRope(nn.Module):
    ...
    def __init__(self, config: PatchTSTConfig):
        ...
        self.temporal_self_attn = PatchTSTRopeAttention(
            ...
            linear_attn=config.linear_attn,
        )
...
class PatchTSTForPrediction(PatchTSTPreTrainedModel):
    ...
    def random_tail_mask(
        self, x: torch.Tensor, min_keep: int, channel_consistent: bool = False
    ) -> torch.Tensor:
        """Mask random tails of variable length from sequences.

        Args:
            x: Input tensor of shape (B, T, D)
            min_keep: Minimum number of timesteps to keep

        Returns:
            masked: Tensor with masked tails
            mask: Boolean mask (True = kept, False = masked)
        """
        B, T, D = x.shape
        keep_lengths = torch.randint(
            min_keep, T + 1, (B, 1 if channel_consistent else D), device=x.device
        )
        mask = torch.arange(T, device=x.device).view(1, -1, 1) < keep_lengths.unsqueeze(
            1
        )
        return mask
