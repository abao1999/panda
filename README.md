...
We provide a script [make_skew_systems.py](scripts/make_skew_systems.py) to search for skew system pairs (driver + response system) that yield valid attractors, using the IC and parameter perturbation samplers. The following command will search through 40 pairs of all the systems in the train split, coupling the phase space, and saving the samples that failed the attractor checks. Setting `num-param-perturbations` and `num_ics` currently samples for the driver and response systems independently. Standardization is a work in progress.
```
...
+ [HIGH | EASY] [QUESTION] figure out how to properly configure eval metrics (e.g. whats the right seasonality for mase?)
