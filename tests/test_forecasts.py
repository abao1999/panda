...
if __name__ == "__main__":
    ...
    if os.path.exists(forecast_dyst_dir):
        ...
        fc_dyst_filepaths = sorted(
            list(Path(forecast_dyst_dir).glob("*.npy")),
        ...
        raise Exception(
            f"Directory {forecast_dyst_dir} does not exist in {FORECAST_DATA_DIR}"
        )
    ...
    for filepath in fc_dyst_filepaths:
        ...
        dyst_coords = np.mean(
            dyst_coords, axis=1
        )  # average along axis=1 to get (dim, prediction_length)
    ...
    fc_dyst_coords_samples = (
        np.array(fc_dyst_coords_samples).squeeze().transpose(0, 2, 1)
    )
    ...
    for filepath in gt_dyst_filepaths:
        ...
        gts_dataset = FileDataset(
            path=Path(filepath), freq="h", one_dim_target=False
    ...
    # should be shape (num_samples, dim, prediction_length)
    gt_dyst_coords_samples = np.array(gt_dyst_coords_samples).squeeze()
    print(gt_dyst_coords_samples.shape)

    dim = gt_dyst_coords_samples.shape[1]
    npts = gt_dyst_coords_samples.shape[2]
    assert (
        dim == fc_dyst_coords_samples.shape[1]
    ), "Mismatch in dimensions of ground truth and forecasted trajectories"
    ...
    print(
        f"Modifying forecasted trajectories to prepend {context_length} context points from ground truth"
    )
    ...
    for i in range(num_samples):
        ...
        full_fc_dyst_coords_samples[i] = np.hstack(
            (gt_coords[:, :context_length], fc_coords)
        )
