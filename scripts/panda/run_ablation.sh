...
for run_name in ${run_names[@]}; do
    echo "Running univariate ablation for $run_name"
    python scripts/panda/ablation.py \
        eval.mode=predict \
        ...
        eval.num_samples_per_subdir=5 \
        eval.window_style=sampled \
        eval.batch_size=64 \
        eval.context_length=512 \
        eval.prediction_length=512 \
        eval.limit_prediction_length=false \
        eval.metrics_save_dir=$WORK/eval_results/patchtst_univariate/$run_name/test_zeroshot \
