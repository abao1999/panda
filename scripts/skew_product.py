...
def get_coupling(sys_driver, sys_response) -> np.ndarray:
    ...
    """
    print(
        f"Computing coupling strength between {sys_driver.name} and {sys_response.name}"
    )
...
def skew_sys(sys_driver, sys_response, affine_map):
    ...
    def _skew_rhs(t, combined_ics):
        ...
        x_driver, x_response = (
            combined_ics[:n_driver],
            combined_ics[n_driver : n_driver + n_response],
        )
...
def run_skew_product_system(sys_driver, sys_response):
    ...
    n_driver, n_response = len(sys_driver.ic), len(sys_response.ic)
    print(
        f"dimension of driver system: {n_driver}, dimension of response system: {n_response}"
    )
